...
SETTINGS = (
    ...
    "PI_SHARED_DIRECT_ONLY", "PI_SHARED_DIRECT_LAUNCHERS", "PI_SHARED_LS99_EXTRAS",
...
def remember_settings(env):
    ...
    for key in SETTINGS:
        ...
        if key in {"PI_SHARED_DIRECT_ONLY", "PI_SHARED_DIRECT_LAUNCHERS", "PI_SHARED_LS99_EXTRAS", "PI_SETUP_NO_SHELL_RC"} and value not in {"0", "1"}:
            raise RuntimeError(f"Invalid boolean setting: {key}")
        if key not in {"MODEL_GATEWAY_LAUNCHD_LABEL", "PI_SETUP_NO_SHELL_RC", "PI_SHARED_DIRECT_ONLY", "PI_SHARED_DIRECT_LAUNCHERS", "PI_SHARED_LS99_EXTRAS"}:
